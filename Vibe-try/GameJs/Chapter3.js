...
${this.game.gameState.playerGender === "male" ? "张伟" : "李娜"}倒在地上，昏迷不醒。祭坛开始崩塌，整个房间摇摇欲坠。`, [
    ...
    escapeWithFriend() {
        this.friendSaved = true;
        this.showDialogue(`你背起${this.game.gameState.playerGender === "male" ? "张伟" : "李娜"}，在祭坛崩塌前找到了出口。你们逃出学校，天空开始泛白。
${this.game.gameState.playerGender === "male" ? "张伟" : "李娜"}慢慢苏醒过来："谢谢你...是你救了我。"
你看着逐渐明亮的天空，知道一切终于结束了。`, [
            { text: '完成第三章', action: () => this.completeChapter() }
        ]);
    }

    findExit() {
        this.showDialogue(`你在祭坛崩塌前找到了出口。你逃出学校，天空开始泛白。
你回头望去，学校在阳光下显得格外平静，仿佛昨晚的一切只是一场噩梦。但你知道，有些事情永远改变了。`, [
            { text: '完成第三章', action: () => this.completeChapter() }
        ]);
    }
...
${friendName}倒在地上，昏迷不醒。`, [
    ...
    leaveWithFriend() {
        this.friendSaved = true;
        this.showDialogue(`你背起${this.game.gameState.playerGender === "male" ? "张伟" : "李娜"}，离开了学校。天空开始泛白，一切似乎都结束了。`, [
            { text: '完成第三章', action: () => this.completeChapter() }
        ]);
    }
